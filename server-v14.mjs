...
import {routeOfferNegotiationV22} from './routes-offer-negotiation-v22.mjs';
import {routeCommercialServicesV23} from './routes-commercial-services-v23.mjs';
import {routeDealerLeadsV24} from './routes-dealer-leads-v24.mjs';
import {routeMarketIntelligencePublicV25} from './routes-market-intelligence-v25.mjs';
import {routeTasteV28} from './routes-taste-v28.mjs';
...
http.createServer(async(req,res)=>{try{const url=new URL(req.url,`http://${req.headers.host||'localhost'}`);if(!url.pathname.startsWith('/api/'))return serve(req,res,url);await prepareProviderWebhookRequest(req,url);if(await routeOrderVerificationPublicV16(req,res,url)!==false)return;if(await routePassportPublicV16(req,res,url)!==false)return;if(await routeInvariantPublicV14(req,res,url)!==false)return;if(await routeE2EPublicV14(req,res,url)!==false)return;if(await routeAuctionResultsPublicV20(req,res,url)!==false)return;if(await routeMarketIntelligencePublicV25(req,res,url)!==false)return;if(await routeSimilarityPublicV18(req,res,url)!==false)return;if(await routeProductPublicV13(req,res,url)!==false)return;if(await routeExperiencePublicV12(req,res,url)!==false)return;if(await routeFoundationPublicV10(req,res,url)!==false)return;if(await routeAuthPublic(req,res,url)!==false)return;const ctx=await authContext(req);if(await routeOrderVerificationV16(req,res,url,ctx)!==false)return;if(await routePassportV16(req,res,url,ctx)!==false)return;if(await routeDisputeV16(req,res,url,ctx)!==false)return;if(await routePublicationV16(req,res,url,ctx)!==false)return;if(await routeInvariantsV14(req,res,url,ctx)!==false)return;if(await routeOwnershipV14(req,res,url,ctx)!==false)return;if(await routeOrganizationsV15(req,res,url,ctx)!==false)return;if(await routeOfferNegotiationV22(req,res,url,ctx)!==false)return;if(await routeCommercialServicesV23(req,res,url,ctx)!==false)return;if(await routeInquiryV21(req,res,url,ctx)!==false)return;if(await routeE2EV14(req,res,url,ctx)!==false)return;if(await routeTasteV28(req,res,url,ctx)!==false)return;if(await routePreferencesV14(req,res,url,ctx)!==false)return;if(await routeMediaV14(req,res,url,ctx)!==false)return;if(await routeExperienceMutationsV12(req,res,url,ctx)!==false)return;if(await routeCollectionMutationsV10(req,res,url,ctx)!==false)return;if(await routeAuctionIntegrityV10(req,res,url,ctx)!==false)return;if(await routeOperatorCockpitV16(req,res,url,ctx)!==false)return;if(await routeOperationsV10(req,res,url,ctx)!==false)return;if(await routeDealerLeadsV24(req,res,url,ctx)!==false)return;if(await routeSellerAnalyticsV17(req,res,url,ctx)!==false)return;if(await routeEngagementV17(req,res,url,ctx)!==false)return;if(await routeMarket(req,res,url,ctx)!==false)return;if(await routeOperatorHooksV14(req,res,url,ctx)!==false)return;if(await routeOperator(req,res,url,ctx)!==false)return;return send(res,ctx?404:401,{error:ctx?'Not found':'Authentication required',code:ctx?undefined:'AUTH_REQUIRED'})}catch(e){console.error(e);return send(res,e.status||500,{error:e.message||'Unexpected server error',code:e.code,minimum:e.minimum,missing:e.missing})}}).listen(PORT,'0.0.0.0',()=>console.log(`ANTIQUA 0.14 listening on ${PORT}; persistence=${db.kind}; runtime=WEB_API`));
