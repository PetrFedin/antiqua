...
function publicListingV13(x,auctionByLot=null){return{...x,purchaseMethod:x.saleType,lot:enrichLot(lot(x.lotId),auctionByLot),seller:seller(x.sellerId)}}
...
export async function routeProductPublicV13(req,res,url){
 if(url.pathname==='/api/health'&&req.method==='GET'){const finance=financeCapabilities();return send(res,200,{status:'ok',service:'antiqua-preview',version:'0.13.0',uiVersion:'0.13.0',preview:PREVIEW,persistence:await db.health(),objectStorage:storageConfig(),auth:{sessions:true,passwordHash:'scrypt',twoFactor:'TOTP',recoveryCodes:true,csrf:true},auctionIntegrity:{engine:db.kind==='POSTGRES'?'POSTGRES_ROW_LOCK':'SERIALIZED_MEMORY_PREVIEW',serverClock:true,idempotencyRequired:true,proxyMaxPrivate:true,authoritativeReads:db.kind==='POSTGRES'},collectionGraph:{collections:true,distributedEnsembles:true,virtualExhibitions:true,iiif:true,linkedArt:true},interaction:{touchFirst:true,directManipulation:true,bottomSheets:true,safeArea:true,reducedMotion:true,dossierCommerce:true,fullscreenGallery:true,pinchZoom:true,dossierNeighbors:true,bilingual:true,nativeBilingualData:true,alignedCards:true,advancedFacets:true,compareObjects:true,productionAccount:true,productionDossier:true,dealerStorefront:true,collectionBuilder:true,exhibitionBuilder:true,ensembleMap:true},payments:{ledgerSchema:finance.ledger,doubleEntry:finance.doubleEntry,payoutHolds:finance.payoutHolds,reconciliation:finance.reconciliation,providerConfigured:finance.providerConfigured},verification:{providerAdapter:true,providerConfigured:Boolean(process.env.KYC_PROVIDER&&process.env.KYC_PROVIDER!=='NOT_CONFIGURED')},time:new Date().toISOString()})}
 if(url.pathname==='/api/catalog'&&req.method==='GET'){const publicAuctions=await listAuthoritativePublicAuctions(),auctionByLot=new Map(publicAuctions.map(a=>[a.lotId,a])),objects=lots.map(o=>enrichLot(o,auctionByLot)),active=[...listings.values()].filter(x=>x.status==='ACTIVE');return send(res,200,{sale:{id:SALE,title:bi('The Collector','Коллекционер'),location:bi('Amsterdam · Online','Амстердам · Онлайн'),status:'LIVE'},lots:objects,auctions:publicAuctions,listings:active.map(x=>publicListingV13(x,auctionByLot)),sellers:[...sellers.values()],facets:buildFacets(objects,active,publicAuctions),catalogueVersion:'0.15'})}
 ...
 const sm=url.pathname.match(/^\/api\/sellers\/([^/]+)$/);if(sm&&req.method==='GET'){const s=seller(sm[1]);if(!s)return send(res,404,{error:'Seller not found'});const publicAuctions=await listAuthoritativePublicAuctions(),auctionByLot=new Map(publicAuctions.map(a=>[a.lotId,a]));return send(res,200,{seller:s,inventory:[...listings.values()].filter(x=>x.sellerId===s.id&&x.status==='ACTIVE').map(x=>publicListingV13(x,auctionByLot))})}
 const pm=url.pathname.match(/^\/api\/lots\/([^/]+)\/passport$/);if(pm&&req.method==='GET'){const raw=lot(pm[1]);if(!raw)return send(res,404,{error:'Object not found'});const aa=await getAuthoritativePublicAuctionForObject(raw.id),auctionByLot=new Map(aa?[[raw.id,aa]]:[]),o=enrichLot(raw,auctionByLot),li=activeListingFor(o.id),ownershipHistory=await publicOwnershipHistory(db,o.id);return send(res,200,{lot:o,commercial:{listing:li?publicListingV13(li,auctionByLot):null,auction:aa},riskFlags:objectRiskFlags(raw),ownershipHistory,disclaimer:bi('Catalogue approval is not a guarantee of authenticity.','Каталожное одобрение не является гарантией подлинности.'),dossier:{permanentPassport:true,evidenceStateModel:true,ownershipHistory:true,iiif:true,linkedArt:true}})}
 return false
}