...
export async function buildTasteProfile(account){
 ...
 for(const fact of objectFacts){if(POSITIVE.has(fact.type)){const p=latestPositive.get(fact.objectId);if(!p||String(fact.at||'')>p)latestPositive.set(fact.objectId,String(fact.at||''))}if(fact.type==='DISMISSED'){const p=latestDismiss.get(fact.objectId);if(!p||String(fact.at||'')>p)latestDismiss.set(fact.objectId,String(fact.at||''))}}
 for(const fact of objectFacts){const o=byId.get(fact.objectId);if(!o)continue;const positiveAfterDismiss=fact.type==='DISMISSED'&&latestPositive.get(fact.objectId)&&latestPositive.get(fact.objectId)>String(fact.at||'');if(positiveAfterDismiss)continue;if(strongExclude.has(fact.type))exclude.add(fact.objectId);for(const d of DIMENSIONS)addFacet(store,d,o[d],fact)}
