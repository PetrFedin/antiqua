...
function buildResult({account,objects,listingRows,draftRows,flags,conversations,offerRows,orderRows,auctionRows,settlements,disputes}){
 const byObject=new Map(objects.map(o=>[o.id,{objectId:o.id,objectCode:o.objectCode||o.objectId||o.id,title:localTitle(o),listingStatus:null,listingPrice:null,currency:o.currency||'EUR',saved:Number(flags[o.id]?.SAVED||0),watching:Number(flags[o.id]?.WATCH||0),conversations:0,offers:0,acceptedOffers:0,orders:0,bids:0,bidders:0,settlements:0,settledValueMinor:0,openDisputes:0}]));
 const ensure=id=>{if(!byObject.has(id)){const o=lot(id)||{id,objectId:id,title:{en:id,ru:id},currency:'EUR'};byObject.set(id,{objectId:id,objectCode:o.objectId||id,title:localTitle(o),listingStatus:null,listingPrice:null,currency:o.currency||'EUR',saved:Number(flags[id]?.SAVED||0),watching:Number(flags[id]?.WATCH||0),conversations:0,offers:0,acceptedOffers:0,orders:0,bids:0,bidders:0,settlements:0,settledValueMinor:0,openDisputes:0})}return byObject.get(id)};
 ...
 for(const x of settlements){const r=ensure(x.objectId);r.settlements++;r.settledValueMinor+=Number(x.winningAmountMinor||0)}
 for(const x of disputes)if(!terminalDispute.has(String(x.status)))ensure(x.objectId).openDisputes++;
 const rows=[...byObject.values()].map(r=>({...r,demandScore:r.saved+r.watching+r.conversations+r.offers+r.bids+r.orders})).sort((a,b)=>b.demandScore-a.demandScore||String(a.objectCode).localeCompare(String(b.objectCode)));
 const summary={objects:rows.length,activeListings:listingRows.filter(x=>x.status==='ACTIVE').length,drafts:draftRows.length,saved:sum(rows,'saved'),watching:sum(rows,'watching'),conversations:sum(rows,'conversations'),offers:sum(rows,'offers'),acceptedOffers:sum(rows,'acceptedOffers'),orders:sum(rows,'orders'),bids:sum(rows,'bids'),bidders:sum(rows,'bidders'),settlements:sum(rows,'settlements'),settledValueMinor:sum(rows,'settledValueMinor'),openDisputes:sum(rows,'openDisputes')};
 return{sellerId:account.sellerId,measurement:{mode:'EVENTS_ONLY',viewsTracked:false,visitorIdentityExposed:false,generatedAt:new Date().toISOString()},summary,portfolio:{listings:statuses(listingRows),drafts:statuses(draftRows)},objects:rows.slice(0,24)};
}
...
async function postgresAnalytics(account){
 ...
 const objectIds=[...new Set([...objects.map(x=>x.id),...listQ.rows.map(x=>x.object_id)])],flags=await aggregateObjectFlags(db,objectIds);
 ...
 return buildResult({account,objects,listingRows,draftRows:draftQ.rows.map(r=>({status:r.status})),flags,
...
async function memoryAnalytics(account){
 const sellerId=account.sellerId,sellerListings=[...listings.values()].filter(x=>x.sellerId===sellerId),objectIds=[...new Set(sellerListings.map(x=>x.lotId))],flags=await aggregateObjectFlags(db,objectIds);
 ...
 return buildResult({account,objects,listingRows,draftRows,flags,conversations,offerRows:[...byOffers].map(([objectId,x])=>({objectId,...x})),orderRows:[...byOrders].map(([objectId,x])=>({objectId,...x})),auctionRows,settlements,disputes});
}
