...
function buildResult({account,objects,listingRows,draftRows,flags,conversations,offerRows,orderRows,auctionRows,settlements,disputes}){
 const byObject=new Map(objects.map(o=>[o.id,{objectId:o.id,objectCode:o.objectCode||o.objectId||o.id,title:localTitle(o),listingStatus:null,listingPrice:null,currency:o.currency||'EUR',saved:Number(flags[o.id]?.SAVED||0),watching:Number(flags[o.id]?.WATCH||0),conversations:0,offers:0,acceptedOffers:0,orders:0,bids:0,bidders:0,settlements:0,settledValueMinor:0,settlementCurrency:null,openDisputes:0}]));
 const ensure=id=>{if(!byObject.has(id)){const o=lot(id)||{id,objectId:id,title:{en:id,ru:id},currency:'EUR'};byObject.set(id,{objectId:id,objectCode:o.objectId||id,title:localTitle(o),listingStatus:null,listingPrice:null,currency:o.currency||'EUR',saved:Number(flags[id]?.SAVED||0),watching:Number(flags[id]?.WATCH||0),conversations:0,offers:0,acceptedOffers:0,orders:0,bids:0,bidders:0,settlements:0,settledValueMinor:0,settlementCurrency:null,openDisputes:0})}return byObject.get(id)};
