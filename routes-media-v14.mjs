...
export async function routeMediaV14(req,res,url,ctx){
 ...
 if(req.method==='POST'&&read[2]==='complete'){
  requireCsrf(req,ctx);const base=String(req.headers['idempotency-key']||`media-complete:${asset.id}`),begun=await beginMediaVerification(a,asset.id,{sourceKey:`${base}:begin`});let verified;try{verified=await verifyUpload({key:begun.asset.storageKey,contentType:begun.asset.contentType,bytes:begun.asset.bytes,sha256:begun.asset.sha256})}catch(e){const rejected=await rejectMediaVerification(asset.id,e,{sourceKey:`${base}:reject`});await audit(req,a,rejected.lifecycle.auditAction,'MEDIA',asset.id,null,{entityType:asset.entityType,entityId:asset.entityId,code:e?.code||'MEDIA_VERIFY_FAILED',lifecycle:rejected.lifecycle});throw e}const result=await markMediaReady(asset.id,verified,{sourceKey:`${base}:ready`}),ready=result.asset;await audit(req,a,result.lifecycle.auditAction,'MEDIA',asset.id,null,{entityType:asset.entityType,entityId:asset.entityId,bytes:ready.bytes,contentType:ready.contentType,lifecycle:result.lifecycle,idempotentTransition:result.idempotentTransition});return send(res,200,{asset:publicAsset(ready),read:await createReadUrl(ready.storageKey)})
 }
 return false
}
