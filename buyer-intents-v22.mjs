...
async function pgOffers(accountId){return(await db.pool.query('SELECT payload FROM offers WHERE buyer_account_id=$1 ORDER BY updated_at DESC,created_at DESC',[accountId])).rows.map(x=>x.payload)}
