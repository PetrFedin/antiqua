...
try{
 ...
 for(const f of ['styles-v07.css','styles-addon-v08.css','styles-addon-v09.css','styles-addon-v10.css','styles-v11.css']){const r=await fetch(`${base}/${f}`);assert.equal(r.status,200);assert.match(r.headers.get('content-type')||'',/text\/css/);const text=await r.text();assert.ok(text.length>100)}
 const ux=await fetch(base+'/ux-v11.js');assert.equal(ux.status,200);assert.match(ux.headers.get('content-type')||'',/javascript/);const js=await ux.text();assert.match(js,/pointerdown/);assert.match(js,/data-bid/);assert.match(js,/sheet-grabber/);assert.match(js,/prefers-reduced-motion|reducedMotion/);
