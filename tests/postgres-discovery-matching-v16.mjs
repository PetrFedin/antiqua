...
const token=crypto.randomUUID().replaceAll('-',''),marker=`Discovery-${token.slice(0,12)}`,objectId=`lot-discovery-${token}`,objectCode=`AQ-DISC-${token.slice(0,10)}`,listingId=`lst-discovery-${token}`,draftId=`draft-discovery-${token}`,sourceKey=`publish-discovery-${token}`;
let subId=null;
