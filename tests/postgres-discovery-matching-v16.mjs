...
const token=crypto.randomUUID().replaceAll('-',''),marker=`Discovery-${token.slice(0,12)}`,objectId=`lot-discovery-${token}`,objectCode=`AQ-DISC-${token.slice(0,10)}`,listingId=`lst-discovery-${token}`,reservedObjectId=`lot-reserved-${token}`,reservedListingId=`lst-reserved-${token}`,auctionObjectId=`lot-auction-${token}`,auctionId=`auc-discovery-${token}`,noPriceObjectId=`lot-no-price-${token}`,draftId=`draft-discovery-${token}`,sourceKey=`publish-discovery-${token}`;
let subId=null;
...
try{
 ...
 const reservedPassport={...passport,id:reservedObjectId,objectId:`AQ-RES-${token.slice(0,8)}`,title:{en:'Reserved semantics object',ru:'Зарезервированный тестовый предмет'}};
 await db.pool.query(`INSERT INTO objects(id,object_code,seller_id,passport,catalogue_status,trust_status,publication_status,passport_hash,created_at,updated_at)
 ...
 const auctionPassport={...passport,id:auctionObjectId,objectId:`AQ-AUC-${token.slice(0,8)}`,title:{en:'Expired auction semantics object',ru:'Завершившийся аукционный тестовый предмет'}};
 await db.pool.query(`INSERT INTO objects(id,object_code,seller_id,passport,catalogue_status,trust_status,publication_status,passport_hash,created_at,updated_at)
 ...
 const noPricePassport={...passport,id:noPriceObjectId,objectId:`AQ-NOP-${token.slice(0,8)}`,title:{en:'No commercial price object',ru:'Предмет без коммерческой цены'},maker:{en:'No Price Maker',ru:'Мастер без цены'}};
 await db.pool.query(`INSERT INTO objects(id,object_code,seller_id,passport,catalogue_status,trust_status,publication_status,passport_hash,created_at,updated_at)
   VALUES($1,$2,'seller-preview',$3,'PUBLISHED','CLEARED','PUBLIC',NULL,now(),now())`,[noPriceObjectId,noPricePassport.objectId,noPricePassport]);
 ...
 const noPrice=await searchDiscoveryPostgres({objectId:noPriceObjectId,priceMin:999999},{limit:20});assert.equal(noPrice.total,1,'legacy price criteria do not exclude objects with no commercial price');
 ...
 const concurrent=await Promise.all([processOutboxBatch({workerId:`discovery-a-${token}`,limit:100,leaseMs:5000}),processOutboxBatch({workerId:`discovery-b-${token}`,limit:100,leaseMs:5000})]);assert.equal(concurrent.reduce((n,x)=>n+x.failed,0),0);
 ...
 await db.pool.query('DELETE FROM objects WHERE id=ANY($1::text[])',[[objectId,reservedObjectId,auctionObjectId,noPriceObjectId]]).catch(()=>{});
 await db.pool.end();
}
