...
const unsoldObjectId='lot-unsold-'+token,unsoldCode='AQ-UNSOLD-'+token,unsoldAuctionId='auc-unsold-'+token,privateObjectId='lot-private-result-'+token,privateCode='AQ-PRIVATE-RESULT-'+token,privateAuctionId='auc-private-result-'+token;
const ended=new Date(Date.now()-3600_000).toISOString(),started=new Date(Date.now()-7200_000).toISOString();

try{
 await db.pool.query("INSERT INTO objects(id,object_code,passport,catalogue_status,trust_status,publication_status) VALUES($1,$2,$3,'APPROVED','CLEARED','PUBLIC'),($4,$5,$6,'APPROVED','CLEARED','PUBLIC'),($7,$8,$9,'APPROVED','CLEARED','PRIVATE')",[
  ...
  privateObjectId,privateCode,{id:privateObjectId,objectId:privateCode,title:{en:'Private result proof',ru:'Непубличный результат'}}
 ]);
 await db.pool.query("INSERT INTO auctions(id,object_id,sale_id,status,current_bid,ends_at,state,baseline_bid,reserve_price,increment,bid_count,leader_account_id,starts_at) VALUES($1,$2,'sale-v20','CLOSED',8500,$3,$4,6000,7000,250,7,$5,$6),($7,$8,'sale-v20','CLOSED',4000,$3,$9,3500,7000,100,3,NULL,$6),($10,$11,'sale-v20','CLOSED',9000,$3,$12,7000,8000,250,5,$5,$6)",[
  ...
  privateAuctionId,privateObjectId,{currency:'EUR',currentBid:9000,bidCount:5,endsAt:ended,startsAt:started,state:'CLOSED'}
 ]);
 ...
 assert.equal(await getPublicAuctionResult(privateAuctionId),null,'PRIVATE object auction result must not be public');
 ...
 await db.pool.query('DELETE FROM auctions WHERE id=ANY($1::text[])',[[auctionId,unsoldAuctionId,privateAuctionId]]).catch(()=>{});
 await db.pool.query('DELETE FROM objects WHERE id=ANY($1::text[])',[[objectId,unsoldObjectId,privateObjectId]]).catch(()=>{});
 await db.pool.end();
}
