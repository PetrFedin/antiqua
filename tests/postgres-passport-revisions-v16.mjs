...
try{
  ...
  await assert.rejects(()=>reviseObjectPassport(operator,objectId,{patch:{media:[{id:otherMediaId,role:'CONDITION',status:'READY'}]},changeKind:'CATALOGUE_CORRECTION',reason:'Attempt to attach unverified media',publicSummary:{en:'Media changed',ru:'Изменены медиа'},sourceKey:`media-no-evidence-${token}`}),e=>e.code==='PASSPORT_EVIDENCE_REQUIRED');
