...
try{
 ...
 assert.equal(migrationManifest.at(-1).version,'019_v28_taste_graph');

 const versions=(await db.pool.query("SELECT version FROM schema_migrations WHERE version IN ('014_v16_discovery_postgres_matching','015_v16_db_integrity','016_v17_object_engagement','017_v22_offer_negotiation','018_v23_condition_viewing','019_v28_taste_graph') ORDER BY version")).rows.map(x=>x.version);
 assert.deepEqual(versions,['014_v16_discovery_postgres_matching','015_v16_db_integrity','016_v17_object_engagement','017_v22_offer_negotiation','018_v23_condition_viewing','019_v28_taste_graph'],'CLI/server migration authority must include 014 through 019');
 ...
 console.log('ANTIQUA v28 PostgreSQL DB integrity: manifest 001..019 + FK/orphan prevention + exact shipment source + cross-aggregate consistency + Money/media guards passed');
}finally{
