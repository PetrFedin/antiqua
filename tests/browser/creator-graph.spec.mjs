...
test('Creator Graph publishes a real profile workflow without bypassing primary-market authority',async({page})=>{
 ...
 auth=await login(page,'BUYER');expect(auth.status).toBe(200);await page.goto('/#creators',{waitUntil:'domcontentloaded'});const card=page.locator('.creator-card').filter({hasText:'Browser Studio '+token});await expect(card).toBeVisible();await card.click();
 const profile=page.locator('.creator-profile');await expect(profile).toBeVisible();await expect(profile).toContainText('Browser Studio '+token);await expect(profile).toContainText(/Первичный рынок|Primary market/i);await expect(profile.locator('[data-passport="lot-109"]')).toBeVisible();
