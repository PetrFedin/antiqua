...
CREATE INDEX IF NOT EXISTS objects_discovery_fts_idx
  ON objects USING gin (
    to_tsvector(
      'simple',
      coalesce(object_code,'') || ' ' ||
      coalesce((passport->'title')::text,'') || ' ' ||
      coalesce((passport->'maker')::text,'') || ' ' ||
      coalesce((passport->'department')::text,'') || ' ' ||
      coalesce((passport->'period')::text,'') || ' ' ||
      coalesce((passport->'origin')::text,'') || ' ' ||
      coalesce((passport->'materials')::text,'') || ' ' ||
      coalesce((passport->'technique')::text,'')
    )
  )
...
CREATE INDEX IF NOT EXISTS auctions_discovery_open_idx
  ON auctions(object_id,ends_at DESC,updated_at DESC);
