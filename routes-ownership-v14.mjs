...
export async function routeOwnershipV14(req,res,url,ctx){
 ...
 if(s.settlementId){const set=await getSettlement(a,s.settlementId);if(set){const fulfilled=set.status==='PAID'?await transitionSettlement(a,set.id,'FULFILLMENT',{shipmentId:s.id,sourceKey:`receipt:${s.id}:fulfillment`}):set,done=fulfilled.status==='FULFILLMENT'?await transitionSettlement(a,fulfilled.id,'COMPLETED',{receiptConfirmed:true,sourceKey:`receipt:${s.id}:complete`}):fulfilled;payoutId=done.metadata?.payoutId||null}}
