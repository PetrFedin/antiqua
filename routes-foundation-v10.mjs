...
export async function routeFoundationPublicV10(req,res,url){
  ...
  if(url.pathname==='/api/health'&&req.method==='GET')return send(res,200,{status:'ok',service:'antiqua-preview',version:'0.10.0',preview:process.env.PREVIEW_MODE!=='false',persistence:await db.health(),auctionIntegrity:{engine:db.kind==='POSTGRES'?'POSTGRES_ROW_LOCK':'SERIALIZED_MEMORY_PREVIEW',serverClock:true,idempotencyRequired:true,proxyMaxPrivate:true},collectionGraph:{collections:true,distributedEnsembles:true,virtualExhibitions:true,iiif:true,linkedArt:true},payments:{ledgerSchema:true,providerConfigured:false},verification:{providerAdapter:true,providerConfigured:false},time:new Date().toISOString()});
